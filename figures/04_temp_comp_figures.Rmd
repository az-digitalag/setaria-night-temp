...
AGB_valid <- chamber_biomass %>%
  ...
  mutate(total_biomass_kg_m2 = ud.convert((stem_DW_g + leaf_DW_g + panicle_DW_g + root_DW_g)/103,
                                          "g/cm2", "kg/m2"),
         total_AGB_kg_m2 = ud.convert((stem_DW_g + leaf_DW_g + panicle_DW_g)/103,
                                          "g/cm2", "kg/m2"),
         day = difftime(harvest_date, sowing_date, units = "days"),
         Trait = "Aboveground~biomass~(kg~m^-2)",
...
all_ts <- rbind.data.frame(biomass_ts, trans_ts) %>%
  mutate(Trait = case_when(trait == "biomass" ~ "Aboveground~biomass~(kg~m^-2)",
...
fig_ts <- ggplot() +
  ...
  geom_ribbon(data = all_ts, aes(day, ymin = lcl_50, ymax = ucl_50, fill = Treatment), 
              alpha = 0.25) +
  # geom_point(data = AGB_valid, aes(x = day, y = total_AGB_kg_m2, color = Treatment)) +
...
biomass_df$sig_05 <- ifelse(biomass_df$p95 < 0, TRUE, FALSE)
biomass_df$sig_025 <- ifelse(biomass_df$p975 < 0, TRUE, FALSE)
...
all_df <- rbind.data.frame(biomass_df, trans_df) %>%
  mutate(Trait = case_when(trait == "biomass" ~ "Delta~Aboveground~biomass~(kg~m^-2)",
